%% TABLE 5 PANEL (b) - FEVD Extraction from 1950-2015 Estimation
%% Drechsel & Tenreyro (2018), "Commodity booms and busts in emerging economies"
%%
%% Run AFTER DTest_1950 estimation completes.
%% This file loads the posterior mode from DTest_1950 and computes FEVD.
%%
%% Strategy: Use estimation with mode_compute=0 (loads saved mode, no re-estimation)
%% and mh_replic=0 (no MCMC), then run stoch_simul for FEVD.

%% LABELING BLOCK
var c, N1, N2, N, k1, k2, k, i1, i2, i, mtil, y, ytil, d,
    w1, w2, rk1, rk2, r, lam,
    lna, lnatil, lng, lns, lnm, lnl, lnptil,
    tb, tbtil, tbagg, gdp, tbaggout, debtout,
    tbout, tbtilout, lngdp, lnc, lni, lngdpfinal, lngdptil,
    y_growth_obs, c_growth_obs, i_growth_obs, tby_obs, ptil_dev_obs;

varexo ea, eatil, eg, es, em, el, eptil;

%% PARAMETER BLOCK
parameters  alphk, alphm, alphktil, del, phi,
            betta, theta, thetatil, om, omtil, gam,
            psi, xi,
            dbar, rstar, s_share, tb_share, nxtil_share,
            abar, rhoa, siga,
...
% Load posterior mode from DTest_1950 estimation (no re-estimation, no MCMC)
% mode_file points to the saved mode from the full estimation
estimation(datafile=DTDATA,first_obs=51,mode_compute=0,mode_file='DTest_1950/Output/DTest_1950_mode',mh_replic=0);

% Compute variance decomposition at the posterior mode
