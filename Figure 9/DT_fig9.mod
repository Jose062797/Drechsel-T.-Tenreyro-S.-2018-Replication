...
% Persistence: calibrated (Table 3 / standard RBC values)
% rhonu lowered from 0.9 to enable hump-shape reversal in panel (g)
rhoa     = 0.9;
rhoatil  = 0.9;
rhog     = 0.9;
rhos     = 0.9;
rhomu    = 0.9;
rhonu    = 0.9;
% Commodity AR(2): calibrated from VAR (same as Figure 4)
rhoptil1 = 0.95;
rhoptil2 = -0.13;

% Volatilities: calibrated (Table 3) — all equal so scaling is consistent
siga    = 0.1;
sigatil = 0.1;
sigg    = 0.1;
sigs    = 0.1;
sigmu   = 0.1;
signu   = 0.1;
sigptil = 0.1064; % calibrated from VAR
...
% IRFs to all shocks (Figure 9)
stoch_simul(order=1,IRF=40) lngdp lnc lni tby_obs lnptil y_growth_obs;
