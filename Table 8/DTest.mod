...
psi = 2.8;
xi = 0.199;             % Based on regression results (positive in calibration, estimated with correct sign)
...
// Table 4 priors (Drechsel & Tenreyro 2018)

// Structural parameters
// CRITICAL FIX: xi must be POSITIVE in code because model equation is r = ... - xi*ptil_dev_obs
// Paper reports negative (~-0.22) but that's the ECONOMIC effect (higher prices → lower rates)
// Code implementation subtracts, so parameter must be positive magnitude
xi,0.2212,0,1,NORMAL_PDF,0.199,0.045;          // n: sensitivity of spread to commodity prices (CORRECTED SIGN)
psi,2.8,0,6,NORMAL_PDF,2.8,0.5;                // x: elasticity of debt in spread

// Commodity price process (AR(2))
rhoptil1,0.8,0.01,0.99,BETA_PDF,0.8,0.10;      // q1_ptil
rhoptil2,0.1278,0.01,0.99,BETA_PDF,0.15,0.10;  // -q2_ptil (positive magnitude, Table 8 posterior mean)

// Other shock persistences (q_i for i = a, atil, g, s, m, l)
...
rhom,0.5,0.01,0.99,BETA_PDF,0.5,0.20;          // Preference shock
rhol,0.5,0.01,0.99,BETA_PDF,0.5,0.20;          // Interest rate risk shock

// Shock volatilities (sigma_i)
// CRITICAL FIX: Mode finder trapped at 0.04 (local minimum). Force start at Table 8 value.
// Strategy: Use mode_compute=4 (gradient optimizer) starting at paper's posterior mean (0.1765)
// This forces exploration around the CORRECT peak instead of wrong local minimum at 0.04
stderr eptil,0.1765,0.001,0.5,INV_GAMMA_PDF,0.10,2;  // Initial=0.1765 from Table 8 (NotebookLM fix)
...
% CRITICAL FIX APPLIED: xi sign corrected (negative → positive)
% SUCCESS: Blanchard-Kahn NOW PASSES ✓
% mode_compute=4 found correct mode (eptil=0.2008, very close to 0.1765!)
% But Hessian singular → trying mode_compute=6 for better numerical Hessian

% Estimation options:
% OPTION 1: Test run (100k draws, ~40 min)
%estimation(datafile=DTDATA,mode_compute=6,mh_replic=100000,mh_nblocks=1,mh_jscale=0.40,mh_drop=0.25);

% OPTION 2: Medium chain (1M draws, 6-10 hours) - ACTIVE
estimation(datafile=DTDATA,mode_compute=6,mh_replic=1000000,mh_nblocks=1,mh_jscale=0.40,mh_drop=0.25);

% OPTION 3: Full replication (10M draws, 2-6 days)
%estimation(datafile=DTDATA,mode_compute=6,mh_replic=10000000,mh_nblocks=1,mh_jscale=0.40,mh_drop=0.25);

% Post-1950 sample version (alternative specification)
...
% NOTE: Commented out when using mode_compute=0 (no mode available)
